...
export class LocalAiClient implements AiProviderStrategy {
  constructor(
  ...
  private async chat(messages: unknown[]): Promise<string> {
    ...
    if (!res.ok) {
      ...
      throw new BadGatewayException(
        `Local AI request failed (${res.status}): ${body}`,
      );
    }
    ...
    if (typeof content !== 'string') {
      throw new BadGatewayException('Local AI response missing message content');
    }
    return content;
  }
  ...
  async detectPanels({
    ...
    if (!Array.isArray(parsed.panels)) {
      throw new BadGatewayException(
        'Local AI panel detection returned no panels array',
      );
    }
    return parsed.panels;
  }

  async redetectPanelBox({
    imageBuffer,
    mimeType,
    previousBox,
    order,
    totalPanels,
  }: RedetectPanelBoxInput): Promise<PanelBox> {
    const messages = [
      {
        role: 'system',
        content:
          'You are a manga page layout analyzer. You previously detected panel bounding boxes for this page, ' +
        ...
        content: [
          {
            type: 'text',
            text:
              `This manga page has ${totalPanels} panels in reading order. ` +
    ...
    const raw = await this.chat(messages);
    const parsed = JSON.parse(raw) as { panel?: PanelBox };
    if (!parsed.panel || typeof parsed.panel !== 'object') {
      throw new BadGatewayException(
        'Local AI panel redetection returned no panel object',
      );
    }
    return parsed.panel;
  }
  ...
  async generateNarration({
    ...
    if (!Array.isArray(parsed.narrations)) {
      throw new BadGatewayException(
        'Local AI narration generation returned no narrations array',
      );
    }
  ...
  async translateTexts({
    ...
    if (!Array.isArray(parsed.translations)) {
      throw new BadGatewayException(
        'Local AI translation returned no translations array',
      );
    }
