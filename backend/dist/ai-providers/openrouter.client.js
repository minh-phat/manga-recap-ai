...
class OpenRouterClient {
    ...
    async chat(messages) {
        ...
        if (!res.ok) {
            ...
            throw new common_1.BadGatewayException(`OpenRouter request failed (${res.status}): ${body}`);
        }
        const data = (await res.json());
        const content = data.choices?.[0]?.message?.content;
        if (typeof content !== 'string') {
            throw new common_1.BadGatewayException('OpenRouter response missing message content');
        }
        return content;
    }
    async detectPanels({ imageBuffer, mimeType, }) {
        ...
        if (!Array.isArray(parsed.panels)) {
            throw new common_1.BadGatewayException('OpenRouter panel detection returned no panels array');
        }
    ...
    async generateNarration({ panels, storySoFar, pageIndex, }) {
        ...
        if (!Array.isArray(parsed.narrations)) {
            throw new common_1.BadGatewayException('OpenRouter narration generation returned no narrations array');
        }
